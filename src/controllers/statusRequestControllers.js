...
const { parseTanggal, formatDate, formatTime } = require("../helper/date");

const EXPIRY_MINUTES = 15;

// ─── Shared helper: terapkan status ke absensiSiswa via detail ───────────────
const applyStatusChange = async (siswa_id, kelas_id, tanggal, walas_id, status_baru, keterangan) => {
    ...
    if (!absensi) {
        absensi = await prisma.absensiSiswa.create({
            ...
            include: { detail: { where: { jadwal_id: null, guru_id: parseInt(walas_id), deleted_at: null } } }
        });
    }
...
const createRequest = async (req, res) => {
    try {
        ...
        // Ambil status_lama dari detail terkini (jadwal_id null = walas entry)
        ...
        const status_lama = existingAbsensi?.detail?.[0]?.status ?? null;
...
// ─── GET /api/v1/status-request/pending?walas_id=X&kelas_id=Y ─────────────
...
// ─── PATCH /api/v1/status-request/:id/respond ─────────────────────────────
...
const respondRequest = async (req, res) => {
    try {
        ...
        if (approved) {
            // Terapkan perubahan status ke absensi
        ...
        return res.json({
            success: true,
            message: approved ? "Permintaan disetujui dan status siswa telah diperbarui" : "Permintaan ditolak",
...
const autoApproveExpired = async () => {
    ...
    for (const p of expired) {
        try {
            await applyStatusChange(p.siswa_id, p.kelas_id, p.tanggal, p.walas_id, p.status_baru, p.keterangan);
            await prisma.permintaanStatusAbsensi.update({
                where: { id: p.id },
...
module.exports = { createRequest, getPendingRequests, respondRequest, autoApproveExpired };
